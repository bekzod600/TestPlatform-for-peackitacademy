...
-- =============================================
-- 14. TEST_QUESTIONS  (many-to-many junction)
-- =============================================
CREATE TABLE IF NOT EXISTS public.test_questions (
  ...
  CONSTRAINT test_questions_test_fkey FOREIGN KEY (test_id)
      REFERENCES tests (id) ON DELETE CASCADE,
  CONSTRAINT test_questions_question_fkey FOREIGN KEY (question_id)
      REFERENCES questions (id) ON DELETE CASCADE
) TABLESPACE pg_default;
...
CREATE INDEX IF NOT EXISTS idx_test_questions_question
  ON public.test_questions USING BTREE (question_id);
...
BEGIN
  ...
  SET is_correct = (
    ...
    AND EXISTS (
      ...
      WHERE ao.id = ta.selected_option_id AND ao.is_correct = true
    )
  )
  ...
  SELECT att.total_questions, COALESCE(SUM(q.points), 0)
  ...
  SELECT COALESCE(SUM(CASE WHEN ta.is_correct = true THEN q.points ELSE 0 END), 0)
  INTO v_earned_score
  FROM public.test_answers ta
